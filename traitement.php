...
if(isset($_POST['submit'])){
  if(!empty($_POST['nom'])&& !empty($_POST['email']) && !empty($_POST['phone'])&& !empty($_POST['commantaire'])){
    ini_set( 'display_errors', 1 );
    error_reporting( E_ALL );
    $from = $_POST['email'];
    $to = "yachaskillb@gmail.com";
    $subject = " objet :". $_POST['commantaire']." tel:".$_POST['phone']."Email:".$_POST['email'];
    $message = "PHP mail ks just finewor";
    $headers = "From:" . $from;
...
if(empty($_POST["nom"])){
$nomErr= "Non Obligatoire";
}else{
$nom=test_input($_POST["nom"]);
if (!preg_match("/^[a-zA-Z-' ]*$/", $nom)) {
$nomErr="Seuls les lettres et espaces sont autorisées";
}
}
...
$insertion= $connexion->prepare("INSERT INTO form(nom,email,phone,commantaire)
             VALUES(:nom,:email,:phone,:commantaire)");
